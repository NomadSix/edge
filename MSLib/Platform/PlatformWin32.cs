...
namespace Microsoft.Xna.Framework.Net {
	public static partial class NetUtility {
		...
		public static IPAddress GetBroadcastAddress () {
			...
			foreach (UnicastIPAddressInformation unicastAddress in properties.UnicastAddresses) {
				if (unicastAddress != null && unicastAddress.Address != null && unicastAddress.Address.AddressFamily == AddressFamily.InterNetwork) {
					var mask = GetMask (unicastAddress.Address);
		...
		public static IPAddress GetMask (IPAddress address) {
			...
			if (firstOctet >= 0 && firstOctet <= 127)
				return new IPAddress(0xFF000000);
			else if (firstOctet >= 128 && firstOctet <= 191)
				return new IPAddress (0xFFFF0000);
			else if (firstOctet >= 192 && firstOctet <= 223)
				return new IPAddress (0xFFFFFF00);
			else return new IPAddress (0x00000000);
		}
