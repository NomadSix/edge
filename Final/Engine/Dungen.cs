...
namespace Edge.Hyperion.Engine {
    public class Dungen : Screen {
        Texture2D pixel, artDebug;
        NetClient atlasClient;
        string Port, Address;
        Color pColor = new Color(1f,1f,1f);

        TileMap Map = new TileMap(@"Map\dungenDemo.csv");
        Point mapSize = new Point(AssetStore.DungenSize);
        Vector2 MoveVector;
        ...
        public Dungen(Game game, string address, string port) : base(game) {
            Port = port;
            Address = address;
            cam.Zoom = 2f;
        }
        ...
        protected override void LoadContent() {
            ...
            Tile.TileSetTexture = that.Content.Load<Texture2D>(@"..\Images\Sheets\Tiles\DungenSheet.png");
            base.LoadContent();
        }
            ...
            foreach (var player in players.Where(x => x.NetID == atlasClient.UniqueIdentifier)) {
                ...
                if (player.HitBox.Intersects(Portal)) {
                    ...
                    if (atlasClient.ServerConnection == null)
                        that.SetScreen(new Town(that, Address, Port));
                }
            }
